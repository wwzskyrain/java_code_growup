...
public class ThreadPool extends ThreadGroup {   //没有实现ThreadGroup的一个方法，干么还"继承"这个ThreadGroup"呢？
    ...
    public synchronized void execute(Runnable task) {
        ...
        if (task != null) {
            workQueue.add(task);        //这个workQueue是无界的，可以接受无限多的任务（理论上）
    ...
    protected synchronized Runnable getTask() throws InterruptedException {
        while (workQueue.size() == 0) { //synchronized  {   while(condition){  wait(); }} 标准用法
    ...
    private class WorkThread extends Thread {
        ...
        public void run() {
            while (!isInterrupted()) {  //isInterrupted()方法继承自Thread类，判断线程是否被中断
                ...
                try { //运行任务，捕获异常
                    task.run(); //run()完了，就再次去getTask()，这一下就要么挂上去，要么立刻就返回了。
                } catch (Throwable t) {
                    t.printStackTrace();
                }
            }//#while
